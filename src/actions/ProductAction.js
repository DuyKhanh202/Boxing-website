...
export const filterProductByType = (name) => async (dispatch) => {
  try {
    const { data } = await axios.get(`${BASE_URL}/api/product/getalishot`);
...
export const getAllProduct = () => async (dispatch) => {
  try {
    const { data } = await axios.post(`http://localhost:8080/api/product/getlist`);
...
export const getproductById = (id) => async (dispatch) => {
  try {
    ...
    const { data }=  await axios.post("http://localhost:8080/api/product/getoneinformation", body);

    dispatch({ type: "GET_PRODUCT_BY_ID", payload: data[0] });
  } catch (error) {
